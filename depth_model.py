...
class DepthEstimator:
    ...
    def colorize_depth(self, depth_map, cmap=cv2.COLORMAP_JET, enhance_contrast=True, focus_range=None,
                        edge_weight=0.2, clahe_clip_limit=2.4):
        """
        Colorize a depth map with contrast enhancement, edge overlay, and local contrast for better feature visibility.

        Args:
            depth_map (numpy.ndarray): Depth map normalized to 0-1
            cmap (int): OpenCV colormap (default: cv2.COLORMAP_JET)
            enhance_contrast (bool): If True, applies global histogram equalization
            focus_range (tuple): (min_depth, max_depth) to emphasize a specific depth range
            edge_weight (float): Weight of edges in the final image (0.0 to 1.0)
            clahe_clip_limit (float): Clip limit for CLAHE (higher values increase local contrast)

        Returns:
            numpy.ndarray: Colorized depth map with edges in BGR format
        """
        # Ensure depth_map is in range 0-1
        depth_map = np.clip(depth_map, 0, 1)

        # Focus on a specific depth range if provided
        if focus_range is not None:
            min_depth, max_depth = focus_range
            depth_map = np.clip((depth_map - min_depth) / (max_depth - min_depth), 0, 1)

        # Convert to 8-bit format
        depth_map_uint8 = (depth_map * 255).astype(np.uint8)

        # Enhance global contrast with histogram equalization if requested
        if enhance_contrast:
            depth_map_uint8 = cv2.equalizeHist(depth_map_uint8)

        # Apply CLAHE (Contrast Limited Adaptive Histogram Equalization) for local contrast
        clahe = cv2.createCLAHE(clipLimit=clahe_clip_limit, tileGridSize=(8, 8))
        depth_map_clahe = clahe.apply(depth_map_uint8)

        # Apply colormap to the enhanced depth map
        colored_depth = cv2.applyColorMap(depth_map_clahe, cmap)

        # Detect edges using Canny on the CLAHE-enhanced depth map
        edges = cv2.Canny(depth_map_clahe, 30, 100)  # Lowered thresholds to capture finer details like the nose
        edges_colored = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)  # Convert edges to BGR

        # Overlay edges onto the colored depth map
        colored_depth = cv2.addWeighted(colored_depth, 1.0 - edge_weight, edges_colored, edge_weight, 0)

        return colored_depth
    #def colorize_depth(self, depth_map, cmap=cv2.COLORMAP_RAINBOW):
        """
