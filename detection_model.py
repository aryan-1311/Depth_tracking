...
class ObjectDetector:
    ...
    """
    def __init__(self, model_size='large', conf_thres=0.4, iou_thres=0.45, classes=None, device=None):
        """
        ...
        model_name = model_map.get(model_size.lower(), model_map['large'])
    ...
    def detect(self, image, track=True):
        ...
        try:
            if track:
                ...
                results = self.model.track(image, verbose=False, device=self.device, persist=True,conf=conf_thres)
            else:
                # Run inference without tracking
                results = self.model.predict(image, verbose=False, device=self.device,conf=conf_thres)
            ...
            if self.device == 'mps' and "not currently implemented for the MPS device" in str(e):
                ...
                if track:
                    results = self.model.track(image, verbose=False, device='cpu', persist=True,conf=conf_thres)
                else:
                    results = self.model.predict(image, verbose=False, device='cpu',conf=conf_thres)
            else:
